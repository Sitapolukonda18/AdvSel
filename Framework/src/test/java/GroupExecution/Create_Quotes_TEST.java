...
public class Create_Quotes_TEST extends BaseClass{

	@Test
	public void CreateQuote() throws EncryptedDocumentException, IOException {
      ...
      // qp.getQuoteEnterDiscount().sendKeys("2");
		...
		WebElement message=qp.getToastMsg();
		wutil.waitForVisibilityOfWebelement(driver, message);
		String msg = message.getText();
	    Boolean status=msg.contains(quotesubject);
	    
	    //HARD ASSERT
	Assert.assertEquals(status, true, "Failed to create quote"+quotesubject); //false
	Reporter.log("successfully created the quote"+quotesubject, true);
	//   

	    //SOFT ASSERT 
	    
	  /* SoftAssert soft=new SoftAssert();
	    soft.assertEquals(status, true, "Failed to create the quote"+quotesubject); //false
	    soft.assertAll();
	  */
